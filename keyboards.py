...
from database import (
    ...
    check_price_product,
    get_list_pools,
    get_list_product_from_pools,
    get_min_pool_price,
    get_product_from_id,
    get_user_list_product,
    ...
    try:
        ...
        if not pools:
            ...
            for pool in pools:
                title = pool[3]
                pool_id = pool[0]
                pool_min_price = await get_min_pool_price(user_id, pool_id)
                pool_min_price_str = str(pool_min_price)

                pool_id = f"pool_{pool_id!s}"

                text_keyboard = f"ПУЛ - {pool_min_price_str} -{title}"
        ...
        for product in products:
            if await check_pool_product(user_id, product[0]) == False:
                ...
                text_keyboard = f"{circle} {round(price[3],2)} - {product[5]} - {title}"
...
async def user_list_product_from_pool_keyboard(user_id, poll_id):
    ...
    try:
        ...
        for product in products:
            ...
            text_keyboard = f"{circle} {round(price[3],2)} - {market} - {product_title}"
            button = InlineKeyboardButton(
        ...
        print(f"An error occurred: ошибка тут {e}")  # Логирование ошибки
        return InlineKeyboardMarkup(
            inline_keyboard=[]
        )  # Возвращаем пустую клавиатуру при ошибке
